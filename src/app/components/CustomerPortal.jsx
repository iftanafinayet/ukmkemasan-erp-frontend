...
export default function CustomerPortal() {
    ...
    const renderDashboard = () => (
        <div className="space-y-8 animate-in fade-in duration-500">
            {/* Welcome */}
            <div className="bg-gradient-to-r from-primary to-primary p-8 rounded-3xl text-white shadow-xl shadow-primary/20">
                <h2 className="text-2xl font-black">Selamat Datang, {user?.name || 'Customer'}! 👋</h2>
                ...
                <button onClick={() => navigate('/portal/orders/create')}
                    className="mt-4 bg-white text-primary px-6 py-3 rounded-2xl font-black text-sm shadow-lg hover:scale-105 active:scale-95 transition-all flex items-center gap-2">
                ...
                <div className="space-y-3">
                    {orders.slice(0, 5).map(order => (
                        <div key={order._id} onClick={() => handleViewOrder(order._id)}
                            className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-primary/20 hover:shadow-sm transition-all cursor-pointer group">
                            <div>
        ...
        return (
            <div className="space-y-6 animate-in fade-in duration-500">
                <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => navigate('/portal/orders/create')}
                        className="bg-primary text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-primary/20 hover:scale-105 active:scale-95 transition-all flex items-center gap-2">
                    ...
                    <div key={order._id} onClick={() => handleViewOrder(order._id)}
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="text-right">
                                ...
                                <ChevronRight size={18} className="ml-auto text-slate-300 group-hover:text-primary transition-colors mt-1" />
                            </div>
                        </div>
                    </div>
                ))}
