...
export function InvoiceModal({
  ...
  return (
    <ModalWrapper onClose={onClose}>
      <div className="mb-8">
        <h3 className="text-2xl font-black text-on-surface tracking-tight">Terbitkan Invoice</h3>
        <p className="text-on-surface-variant text-sm font-medium">Pilih order yang siap ditagihkan ke customer.</p>
      </div>
      <form onSubmit={onSubmit} className="space-y-5">
        <Field label="Order">
          <select
            ...
            className="w-full px-5 py-4 bg-surface-container-low border border-outline-variant rounded-2xl outline-none cursor-pointer focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all text-on-surface font-bold"
          >
        ...
        {selectedOrderForInvoice && (
          <div className="rounded-2xl border border-outline-variant/30 bg-surface-container-low p-4">
            <p className="text-[10px] font-black uppercase tracking-[0.22em] text-muted">Ringkasan Order</p>
        ...
        <Field label="Catatan">
          <textarea
            ...
            className="w-full px-5 py-4 bg-surface-container-low border border-outline-variant rounded-2xl outline-none cursor-pointer focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all text-on-surface font-bold resize-none"
...
export function PaymentModal({
  ...
  return (
    <ModalWrapper onClose={onClose}>
      <div className="mb-8">
        <h3 className="text-2xl font-black text-on-surface tracking-tight">Payment Receive</h3>
        <p className="text-on-surface-variant text-sm font-medium">Catat pembayaran yang masuk dari customer.</p>
      </div>
      <form onSubmit={onSubmit} className="space-y-5">
        <Field label="Invoice">
          <select
        ...
        {selectedInvoiceForPayment && (
          <div className="rounded-2xl border border-outline-variant/30 bg-surface-container-low p-4">
            <p className="text-[10px] font-black uppercase tracking-[0.22em] text-muted">Ringkasan Invoice</p>
            <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
...
export function ReturnModal({
  ...
  return (
    <ModalWrapper onClose={onClose}>
      <div className="mb-8">
        <h3 className="text-2xl font-black text-on-surface tracking-tight">Sales Return</h3>
        <p className="text-on-surface-variant text-sm font-medium">Catat barang yang dikembalikan customer dan restock jika perlu.</p>
      </div>
      <form onSubmit={onSubmit} className="space-y-5">
        <Field label="Order">
          <select
        ...
        <div className="rounded-2xl border border-outline-variant px-5 py-4 bg-surface-container-low space-y-4">
          <label className="flex items-center justify-between gap-4 cursor-pointer">
            <div>
              <p className="font-black text-on-surface text-sm">Kembalikan ke inventory</p>
              <p className="text-xs text-on-surface-variant">Aktifkan jika stok retur perlu ditambahkan kembali ke gudang.</p>
            </div>
            <input
              type="checkbox"
          ...
          {returnForm.restocked && (
            <Field label="Gudang Tujuan">
              <select
                ...
                className="w-full px-5 py-4 bg-surface-container-lowest border border-outline-variant rounded-2xl outline-none cursor-pointer focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all text-on-surface font-bold"
              >
